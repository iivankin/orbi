...
pub(super) fn run_on_macos(
    ...
    if let Some(kind) = trace {
        ...
        let launch_command = vec![launch_target];
...
pub(super) fn run_on_device(
    ...
    if let Some(kind) = trace {
        bail!(
            "`orbi run --trace --device` is temporarily unavailable after the Orbi trace runtime cutover; use simulator or macOS tracing for now (requested {} trace on {})",
            kind.trace_label(),
            device.name()
        );
    }
